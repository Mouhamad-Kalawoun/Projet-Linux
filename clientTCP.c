...
{ int st,af,type,prot;
  ...
  mes = (char *) malloc(256);
  printf("Programme Client TCP, ce code est prepare par Mouhamad Nour Aldine Kalawoun\n");
  printf("Entrer l'adresse du serveur sous la forme a.b.c.d:\n");
...
// "inet_aton" Convertit les adresses IP à partir d'une chaîne de points et numéros à un in_addr struct et retour.
    if (inet_aton(adrserveur, &serveur.sin_addr)!=0)// il doit retourner une valeur differente que 0.
      ...
      printf("Adresse invalide, recommencer ou fin pour terminer: ");
   } while (fin==0);
  if (fin==2) {
